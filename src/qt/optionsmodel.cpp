...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::TUTL);
    ...
    if (settings.contains("nAnonymizeTutelaAmount") || reset)
        SoftSetArg("-anonymizetutelaamount", settings.value("nAnonymizeTutelaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tutela.cpp during shotdown to also remove the window positions
