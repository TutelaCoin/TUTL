...
#include "qt/tutela/forms/ui_masternodeswidget.h"
#include "qt/tutela/qtutils.h"
#include "qt/tutela/mnrow.h"
#include "qt/tutela/mninfodialog.h"

#include "qt/tutela/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(TutelaGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i TUTL requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 10,000 TUTL required."));
            return;
        }
        showHideOp(true);
