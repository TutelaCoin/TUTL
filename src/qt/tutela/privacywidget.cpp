...
#include "qt/tutela/privacywidget.h"
#include "qt/tutela/forms/ui_privacywidget.h"
#include "qt/tutela/qtutils.h"
#include "guiutil.h"
#include "qt/tutela/denomgenerationdialog.h"
#include "qt/tutela/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(TutelaGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zTUTL anonymizes your TUTL by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zTUTL or convert back to TUTL"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 TUTL ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zTUTL");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zTUTL");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zTUTL");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zTUTL");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zTUTL");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zTUTL");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zTUTL");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zTUTL");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zTUTL Movements"));
    ...
    // Only Convert to TUTL enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzTUTL->setTitleClassAndText("btn-title-grey", "Total 0 zTUTL");
    ui->btnTotalzTUTL->setSubTitleClassAndText("text-subtitle", "Show denominations of zTUTL owned.");
    ui->btnTotalzTUTL->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select TUTL outputs to mint into zTUTL.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zTUTL");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzTUTL, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zTUTL");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of TUTL to mint into zTUTL"));
    }else{
        btnText = tr("Convert back to TUTL");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zTUTL to convert back into TUTL"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzTUTL->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzTUTL->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zTUTL is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zTUTL, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zTUTL").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zTUTL minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zTUTL converted back to TUTL"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any TUTL to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTUTL </b>";
    ...
    ui->btnTotalzTUTL->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
