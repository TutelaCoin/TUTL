    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zTUTL Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zTUTL.";
            else if (!mint.isSeedCorrect)
                strReason = "The zTUTL seed used to mint this zTUTL is not the same as currently hold in the wallet";
            else
