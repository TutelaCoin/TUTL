...
#include "qt/tutela/send.h"
#include "qt/tutela/forms/ui_send.h"
#include "qt/tutela/addnewcontactdialog.h"
#include "qt/tutela/qtutils.h"
#include "qt/tutela/sendchangeaddressdialog.h"
#include "qt/tutela/optionbutton.h"
#include "qt/tutela/sendconfirmdialog.h"
#include "qt/tutela/myaddressrow.h"
#include "qt/tutela/guitransactionsutils.h"
...
SendWidget::SendWidget(TutelaGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (TUTL)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("Tutela address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 TUTL");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_TUTL : AskPassphraseDialog::Context::Send_zTUTL, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zTUTL is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zTUTL selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zTUTL transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZTUTL_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zTUTL require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zTUTL transaction
        ...
        emit message("zTUTL transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isTUTL){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zTUTL in your balance to select."));
        }
    }
}
...
void SendWidget::onTUTLSelected(bool _isTUTL){
    isTUTL = _isTUTL;
    setCssProperty(coinIcon, _isTUTL ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
