...
public:
    ...
    bool WriteZTUTLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZTUTLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZTUTLSeed_deprecated(uint256& seed);
    bool EraseZTUTLSeed();
    bool EraseZTUTLSeed_deprecated();

    bool WriteZTUTLCount(const uint32_t& nCount);
    bool ReadZTUTLCount(uint32_t& nCount);
