...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected TUTL and zTUTL supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zTUTLsupply']:
                assert_equal(node['zTUTLsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check TUTL and zTUTL supply at the beginning
        # ------------------------------------------
        # zTUTL supply: 2 coins for each denomination
        ...
        # TUTL supply: block rewards minus burned fees for minting
        ...
        # Verify that TUTL and zTUTL supplies were properly updated after the spends and reorgs
        self.log.info("Check TUTL and zTUTL supply...")
