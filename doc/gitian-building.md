...
*Setup instructions for a gitian build of Tutela Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Tutela
...
to the Tutela GitHub release page.
...
- [Building Tutela Core](#building-tutela-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/tutela-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/tutela-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/tutela-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/tutela-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/tutela-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/tutela-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/TutelaCoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@tutela.io) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/tutela-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/tutela-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/tutela-osx-*-build.assert
```
